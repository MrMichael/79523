...
**大厅标签内容：**
- **用户列表**：全部注册用户，每行 `用户名 + 在线/离线徽标 + 胜局 + 拳王`。
- **房间列表**：每行 `房号 · 房主 · 当前人数/6 · 等待中|进行中`；**等待中**房间可点「加入」，**进行中**置灰不可加入；房号照旧可直接输入加入。
- **创建房间**（无人数选择）、**房号加入**。
- 实时刷新：服务端向已鉴权连接广播 `lobby_users_updated` / `lobby_rooms_updated`（用户上下线、房间创建/加入/离开/解散时触发）。

## 10. 全局排名（需求 4）

- `GET /api/leaderboard?metric=wins|boxerWins`（需鉴权）。
- **双榜切换**：
  - 胜局榜：`ORDER BY wins DESC, boxer_wins DESC`
  - 拳王榜：`ORDER BY boxer_wins DESC, wins DESC`
- 每行显示 `名次 · 用户名 · 胜局 · 拳王 · 在线点`。数据源为 DB 查询。
